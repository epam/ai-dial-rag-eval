...
evaluation_result_1 = st.sidebar.selectbox(
    "First evaluation", datasets_list, index=0
)

evaluation_result_2 = st.sidebar.selectbox(
    "Second evaluation", datasets_list, index=1
)
...
with st.spinner("Loading evaluation results..."):
    ...
    comparison_data = evaluation_result_1_data[merge_columns + common_columns + comparison_columns].merge(
...
for column in ["context_1", "context_2"]:
    comparison_data[column] = comparison_data.apply(
        lambda row: highlight_context_by_facts(row, context_column=column),
        axis=1
    )
...
if show_as_highlighted_table:
    highlighted = comparison_data.style.set_properties(width="5%", **{"text-align": "left", "escape": False})
