...
questions_path = st.selectbox(
    "Questions",
    get_questions_datasets_list()
)
...
document_url = st.text_input("Document URL", value="https://d18rn0p25nwr6d.cloudfront.net/CIK-0001352010/7ea14ea9-d8f7-4039-8342-674cfbece898.pdf")
#st.page_link(document_url, label="Open document")


if not "collected_answers" in st.session_state:
...
if st.button("Collect answers", type="primary"):
    st.session_state.collected_answers = questions_data.progress_apply(
        lambda row: ask_selected_app(row["question"], document_url),
        axis=1
    )
...
suggested_name = questions_path.display_name.replace(".parquet", f"_answers_{selected_app_key}.parquet")
...
if st.button(
    "Save answers",
    type="primary",
    disabled=st.session_state.collected_answers.empty
):
    try:
