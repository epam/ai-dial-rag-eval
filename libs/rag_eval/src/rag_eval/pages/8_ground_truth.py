...
if not "facts" in st.session_state:
    st.session_state.facts = pd.DataFrame()

if st.button("Extract facts", type="primary"):
    result = answers_data_selected.progress_apply(
        extract_facts_raw_mixtral, axis=1
    )
    st.session_state.facts = pd.DataFrame(
        {
...
if not st.session_state.facts.empty:
    ...
    ground_truth_preview = pd.DataFrame({
        ...
        "ground_truth_facts": st.session_state.facts["facts"].apply(lambda x: "\n\n".join(x if x is not None else [])),
    ...
    ground_truth_data = ground_truth_data[ground_truth_data["validation_result"] == "valid"]
    ground_truth_data.drop(columns=["validation_result"], inplace=True)


    suggested_name = answers_path.display_name.replace(".parquet", "_ground_truth.parquet")
    ...
    if st.button(
        "Save valid facts",
        type="primary",
        disabled=ground_truth_data.empty
    ):
        try:
