...
if "questions_data_imported" not in st.session_state:
...
with st.expander("Import file", expanded=False):
    uploaded_file = st.file_uploader(
        "Choose a file", accept_multiple_files=False, type=list(FILE_READERS.keys())
    )
    if uploaded_file is not None:
        df = read_file_by_extension(uploaded_file)
        st.dataframe(df)
        suggested_question_index = (
            df.columns.get_loc("question") if "question" in df.columns else 0
        )
...
st.session_state.questions_data_edited = st.data_editor(
    ...
    num_rows="dynamic",
    column_config={"question": st.column_config.TextColumn(required=True)},
)


if "dataset_name" not in st.session_state:
    st.session_state.dataset_name = (
        f"questions_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.parquet"
    )

st.session_state.dataset_name = st.text_input(
    "Question dataset name", value=st.session_state.dataset_name
)
...
if st.button(
    ...
    try:
        ...
        write_questions(
            st.session_state.dataset_name, questions_data_edited, rewrite=overwrite
        )
