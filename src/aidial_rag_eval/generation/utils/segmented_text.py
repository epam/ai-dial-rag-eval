...
class SegmentedText:
    ...
    def from_text(cls, text: Text) -> "SegmentedText":
        ...
        segmented_text.apply_splitter_to_segments(splitters[0])
        for splitter in splitters[1:]:
            segmented_text.apply_splitter_to_segments(
                splitter, lambda x: len(x) > max_len
            )

        segmented_text.join_segments_to_next_by_condition(lambda x: len(x) < min_len)
        return segmented_text

    def apply_splitter_to_segments(
        self, splitter: Splitter, checker: Optional[SegmentChecker] = None
    ):
        if checker is None:
        ...
        for i, original_segment in enumerate(self.segments):
            ...
            if i < len(self.delimiters):
                new_delimiters.append(self.delimiters[i])

        self.segments = new_segments
        self.delimiters = new_delimiters

    def join_segments_to_next_by_condition(self, checker: SegmentChecker):
        ...
        for i in reversed(range(len(self.segments) - 1)):
            if checker(self.segments[i]):
                reversed_segments[-1] = join_with_delimiters(
                    [self.segments[i], reversed_segments[-1]],
                    [self.delimiters[i]],
                )
            else:
                reversed_segments.append(self.segments[i])
                reversed_delimiters.append(self.delimiters[i])
        ...
        self.segments = reversed_segments[::-1]
        self.delimiters = reversed_delimiters[::-1]
